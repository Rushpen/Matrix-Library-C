...
START_TEST(s21_mult_matrix_test_1) {
  ...
  prec_res.matrix[0][0] = 1;
  prec_res.matrix[0][1] = 2;
  prec_res.matrix[1][0] = 3;
  prec_res.matrix[1][1] = 4;

  error += s21_mult_matrix(&A1, &A2, &result);
...
START_TEST(s21_mult_matrix_test_2) {
  ...
  int error = 0;
  error += s21_create_matrix(2, 2, &A1);
  error += s21_create_matrix(2, 1, &A2);
...
START_TEST(s21_mult_matrix_test_3) {
  ...
  error += s21_mult_matrix(&A1, NULL, &result);
  ck_assert_int_eq(error, 1);
  s21_remove_matrix(&A1);
}
END_TEST

Suite* s21_mult_matrix_suite_create(void) {
  Suite* suite = suite_create("s21_mult_matrix_suite");
  TCase* tcase_core = tcase_create("s21_mult_matrix_tcase");
