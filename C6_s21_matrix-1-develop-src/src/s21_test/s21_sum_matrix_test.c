...
START_TEST(s21_sum_matrix_test_1) {
  ...
  error += s21_sum_matrix(&A1, &A2, &result);
...
START_TEST(s21_sum_matrix_test_4) {
  ...
  error += s21_sum_matrix(&A1, NULL, &result);
  ck_assert_int_eq(error, 1);
  s21_remove_matrix(&A1);
}
END_TEST

Suite* s21_sum_matrix_suite_create(void) {
  Suite* suite = suite_create("s21_sum_matrix_suite");
  TCase* tcase_core = tcase_create("s21_sum_matrix_tcase");
